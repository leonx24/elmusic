...
export default class SkipCommand extends Command {
  ...
  async run(client: BotClient, interaction: ChatInputCommandInteraction): Promise<unknown> {
    ...
    if (!queue.autoplay && queue.songs.length <= 1) {
      ...
      return interaction.reply(
        MusicEmbedBuilder.success("Stopped", `Skipped **${currentSong.name}** and ended the queue.`)
      );
    }
    ...
    return interaction.reply(
      MusicEmbedBuilder.success("Skipped", `Skipped the current track: **${currentSong.name}**`)
    );
  }
}
