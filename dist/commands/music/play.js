...
export default class PlayCommand extends Command {
    ...
    async autocomplete(client, interaction) {
        ...
        try {
            ...
            const response = await fetch(`https://suggestqueries.google.com/complete/search?client=youtube&ds=yt&q=${encodeURIComponent(focusedValue)}`, {
                headers: {
            ...
            if (response.ok) {
                ...
                if (jsonMatch && jsonMatch[1]) {
                    ...
                    const choices = results.slice(0, 10).map((item) => {
