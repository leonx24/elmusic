...
export default class PlayCommand extends Command {
    ...
    async run(client, interaction) {
        ...
        try {
            ...
            if (result?.loadType === "error") {
                ...
                return interaction.editReply(MusicEmbedBuilder.error(`Failed to load track from Lavalink.\n\n**Error:** ${errMsg}\n\nThis usually means the YouTube plugin cannot stream this video. Try a different track, or check the bot logs for client-level errors.`));
            }
            if (!result || !result.data || (Array.isArray(result.data) && result.data.length === 0) || result.loadType === "empty") {
                return interaction.editReply(MusicEmbedBuilder.error("Could not resolve or play this track from YouTube. The source video or playlist may require login or age verification."));
            }
    ...
    async autocomplete(client, interaction) {
        ...
        try {
            ...
            const choices = tracks.slice(0, 10).map((track) => {
